...
#define MOTOR3_IN2_PIN 16
#define ENCODER1_PIN_A 3
...
void computePID(){
  ...
  if(abs(pos3 - targetpos3) > 10){
    motor3PID.Compute();
    analogWrite(MOTOR3_PWM_PIN, abs(output2));
  }
  if(abs(pos2 - targetpos2) > 10){
    motor2PID.Compute();
    analogWrite(MOTOR2_PWM_PIN, abs(output3));
  }
}
...
// Function to receive data from the leader
